    ...
    <section className="py-24 bg-[#faf9f6] w-full overflow-hidden">
      ...
      <div className="w-full flex overflow-x-auto hide-scrollbar px-4 md:px-8 snap-x snap-mandatory gap-4 md:gap-6 pb-6">
        {categories.map((category) => {
          const maxDiscount = category.products.reduce((max, p) => Math.max(max, p.discount || 0), 0);
          
          return (
            <div key={category.id} className="snap-start shrink-0 w-[85%] sm:w-[45%] md:w-[calc(33.33%-1.2rem)] lg:w-[calc(22%-1.2rem)] xl:w-[calc(20%-1.2rem)] flex-none">
              <Link
                href={`/category/${category.slug}`}
                className="flex items-center bg-white border border-gray-200 rounded-full pr-4 md:pr-6 p-1.5 gap-3 md:gap-4 hover:border-[#9c7b94]/50 hover:shadow-lg transition-all w-full group relative"
              >
                ...
                <div className="flex flex-col items-start gap-1 py-1.5 overflow-hidden flex-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {maxDiscount > 0 && (
                      <span className="text-[10px] md:text-[11px] font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-md border border-rose-100 whitespace-nowrap">
